...
parser = argparse.ArgumentParser(description='Download CIMISS METAR data.')
parser.add_argument('-o', '--root-dir', dest='root_dir', default='/data/raw/cimiss', help='Root directory to store METAR data.')
...
ftp_get(f'SURF_CHN_MAIN_MIN/{args.time.format("%Y%m%d%H%M")}.xml', fatal=True)
if args.time.minute == 0:
	ftp_get(f'SURF_CHN_MUL_HOR/{args.time.subtract(hours=1).format("%Y%m%d%H00")}.xml', fatal=False)
