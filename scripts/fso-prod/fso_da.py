...
if currDate.strftime("%H%M") in currentTime_fso  :   # It is time to play
        ...
        # Prepared fc_path
        fc_path_dir = os.path.join(fc_path, ccyymmddhh)

        if os.path.exists(fc_path_dir):
           shutil.rmtree(fc_path_dir)

        try:
           os.makedirs(fc_path_dir)
        except:
           pass
        ...
        rc = bash_cmd(["./wrapper_run_fso_v3.4.ksh", "da", ccyymmddhh, os.environ['NPE'], os.environ['NPE']])
        if not rc==0: sys.exit(1)

        #wait_file( os.path.join(fc_path_dir, 'wrfinput_d01')) 
        #wait_file( os.path.join(fc_path_dir, 'wrfbdy_d01')) 
