...
if currDate.strftime("%H%M") in currentTime_fso:
        ...
        if not os.path.exists( os.path.join(rc_path_dir, "wrfinput_d01") ) or \
           not os.path.exists( os.path.join(rc_path_dir, "wrfbdy_d01") ): 
           print (" RC wrfinput_d01 or wrfbdy_d01 is not available at %s") % rc_path_dir 
           sys.exit(1)
        print ("Found the RC wrfinput and wrfbdy at %s") % rc_path_dir
