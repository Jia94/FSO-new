...
if currDate.strftime("%H%M") in currentTime_fso  :   # It is time to play
        ...
        rc = bash_cmd(["./wrapper_run_fso_v3.4.ksh", "adj", ccyymmddhh, os.environ['NPE'], os.environ['NPE']])
        if not rc == 0 : sys.exit(1)

        wait_file( os.path.join(FSO_PATH, 'run', ccyymmddhh, 'sensitivity', 'wrf_xa', 'init_sens_d01_'+ccyy+'-'+mm+'-'+dd+'_'+hh+':00:00')) 
