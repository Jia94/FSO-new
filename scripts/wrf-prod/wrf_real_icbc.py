...
if currDate.strftime("%H%M") in currentTime  :   # It is time to play
	...
	WRF_namelist (os.path.join(STATIC_DIR, "namelist.input"), "namelist.input", futureStartDate, futureEndDate, fcInterval)
        ...
        # run real.exe to get the wrfinput_d01 and wrfbdy_d01
	   ...
	   os.makedirs(rc_path_dir)
        ...
        shutil.move("wrfinput_d01", os.path.join(rc_path_dir, "wrfinput_d01")) 
        shutil.move("wrfbdy_d01", os.path.join(rc_path_dir, "wrfbdy_d01")) 
        print (" Save wrfinput and wrfbdy_d01 at  %s ") % rc_path_dir
